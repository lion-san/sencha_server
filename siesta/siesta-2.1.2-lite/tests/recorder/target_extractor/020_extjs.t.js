        ...
        t.chain(
            function (next) {
                document.onclick = function (event) {
                    ...
                    t.isDeeply(targets[ 0 ], { type : 'csq', target : '[itemId=win] header[title=foo] => .x-header-text-container', offset : null }, "Correct best target")
                    // css query is placed above the component query, because it points to target element more precisely
                    t.isDeeply(targets[ 1 ], { type : 'css', target : '.x-header-text-container.x-window-header-text-container', offset : null }, "Correct best target")
                    t.isDeeply(targets[ 2 ], { type : 'cq', target : '[itemId=win] header[title=foo]', offset : null }, "Correct best target")
                }
